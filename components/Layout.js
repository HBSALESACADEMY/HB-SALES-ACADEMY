...
export default function Layout({ children, fullBleed }) {
  ...
  const [neueTeamZiele, setNeueTeamZiele] = useState(cachedBadges?.neueTeamZiele ?? 0);
  ...
  // Bei "Systemeinstellung" (siehe pages/settings.js, lib/theme.js): wechselt
  // das Betriebssystem-Theme während der Sitzung, data-theme live nachziehen
  // UND das Branding neu anwenden — sonst bliebe eine eigene, für Dunkel
  // gewählte Hintergrundfarbe der Organisation fälschlich im hellen Theme
  // aktiv (siehe applyOrgBranding-Kommentar in lib/orgBranding.js).
  useEffect(() => watchSystemTheme((resolved) => {
    if (cachedOrg) applyOrgBranding(cachedOrg);
    setDefaultLogo(defaultLogoSrc(resolved));
  }), []);
  ...
  // Start-Wert passend zum Server-Rendering (immer dunkel), direkt nach dem
  // Mount korrigiert.
  ...
  useEffect(() => {
    ...
    async function loadUnread() {
      ...
      // Neue Wochenziele in den eigenen Teams. Nur die laufende Woche und
      // nur fremde Ziele — wer selbst eines setzt, braucht dazu keinen
      // Hinweis (gleiche Regel wie bei der Community).
      let zieleCount = 0;
      ...
      if (mounted) setNeueTeamZiele(zieleCount);
            ...
            <div className="overflow-hidden">
              ...
              {items.map((item) => {
                ...
                const badgeCount = item.key === "community" ? unreadCommunity
                  ...
                  // Über die Route statt nur den Schlüssel: die Einträge der
                  // Navigation liegen in der Datenbank, der Schlüssel kann
                  // je Installation abweichen.
                  : (item.key === "team" || item.route === "/team") ? neueTeamZiele
                  : 0;
        ...
        <div className="flex-1 overflow-y-auto flex flex-col gap-0.5">
          ...
          {(profile?.role === "manager" || profile?.is_admin || profile?.is_platform_admin) && (
            <div className="px-1 pt-1">
              {!showNewTabForm ? (
                <button onClick={() => setShowNewTabForm(true)} className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[13.5px] font-medium text-left w-full text-textMuted hover:bg-surfaceRaised hover:text-textMain border border-dashed border-line">
                  + Neuer Reiter
                </button>
              ) : (
