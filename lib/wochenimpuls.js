...
export function zahlenBlock(zahlen, vorher) {
  ...
  return KENNZAHLEN
    ...
    .map((k) => {
      // Fehlt eine Kennzahl im übergebenen Satz, ist sie null — "undefined"
      // in einer Nachricht an einen Menschen wäre schlimmer als eine 0.
      const heute = Number(z[k.key]) || 0;
      const frueher = Number(v[k.key]) || 0;
      const pfeil = heute > frueher ? " ↑" : heute < frueher ? " ↓" : " →";
      return `${k.label}: ${heute} (Vorwoche: ${frueher})${pfeil}`;
    });
}
 ...
 *
 * Wichtig ist hier, was der Buddy NICHT tun soll: die Zahlen der Person in
 * jede Antwort ziehen. Wer fragt "Wie komme ich am Vorzimmer vorbei?", will
 * eine Antwort auf die Frage — und nicht hören, dass er diese Woche 40
 * Anwahlen hatte. Die Zahlen stehen deshalb ganz am Ende und ausdrücklich
 * als Hintergrund, der nur auf Nachfrage vorkommt.
 */
export function gespraechsAnweisung({ name = "", organisation = "", zahlen, vorher }) {
  return [
    ...
    "",
    "Antworte immer auf DAS, was gefragt ist — wie ein Kollege, der es kann.",
    "Bei einer fachlichen Frage (Einstieg, Vorzimmer, Einwand, Preis, Abschluss, Nachfassen, Struktur, Umgang mit Absagen)",
    "gibst du eine fachliche Antwort: der Gedanke dahinter in einem Satz, dann ein bis drei Sätze, die man am Telefon",
    "wörtlich sagen kann, in Anführungszeichen. Kein Rückgriff auf Zahlen, keine Überleitung dorthin.",
    "Geht es um Persönliches oder um Druck, nimm es ernst, statt es wegzumotivieren — und ohne Zahlen.",
    "",
    "Antworte kurz und konkret: höchstens 8 Sätze, lieber weniger. Keine Aufzählungen mit Sternchen, kein Markdown.",
    ...
    "",
    "Nur als Hintergrund, falls es wirklich um seine Zahlen geht (diese Woche, Vorwoche in Klammern):",
    ...zahlenBlock(zahlen, vorher).map((z) => `- ${z}`),
    "Diese Zahlen erwähnst du NICHT von dir aus. Nur, wenn ausdrücklich danach gefragt wird",
    "(\"wie stehe ich da?\", \"wie war meine Woche?\") oder wenn die Frage ohne sie nicht zu beantworten ist.",
  ].join("\n");
}
