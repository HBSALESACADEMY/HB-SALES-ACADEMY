...
export function stufenAuswertung(leads = []) {
  ...
  leads.filter(istKundentermin).forEach((l) => {
    ...
    let schonKunde = false;
    (Array.isArray(l.stufen_verlauf) ? l.stufen_verlauf : []).forEach((v) => {
      const z = zaehler[v.art] || zaehler.unbestimmt;
      z.gesamt += 1;
      z.wahrgenommen += 1;
      if (v.ergebnis === "kunde" && !schonKunde) { z.kunden += 1; schonKunde = true; }
    });

    // Und die Stufe, auf der er gerade steht. Der Kunde wird dort gezählt,
    // wo er gewonnen wurde — das "kunde", das beim Check-in mitwandert,
    // ist derselbe Kunde und kein zweiter.
    const z = zaehler[artVon(l).key];
    z.gesamt += 1;
    if (l.status === "wahrgenommen") z.wahrgenommen += 1;
    if (l.outcome === "kunde" && !schonKunde) z.kunden += 1;
  });
 ...
 */
/**
 * Ist dieser Kontakt Kunde geworden — egal, auf welcher Stufe er gerade steht?
 *
 * "Kunde geworden" ist kein Ergebnis EINES Gesprächs wie "überlegt noch",
 * sondern ein Zustand des Kontakts. Er hält über die Stufen hinweg: Wer den
 * Check-in plant, hat den Kunden nicht wieder verloren.
 *
 * Der Blick in den Verlauf fängt die Einträge auf, die vor dieser Regel
 * zum Check-in weitergerückt sind — bei ihnen steht das "kunde" nur noch
 * in der abgeschlossenen Stufe.
 */
export function istKundeGeworden(lead) {
  if (lead?.outcome === "kunde") return true;
  return (Array.isArray(lead?.stufen_verlauf) ? lead.stufen_verlauf : [])
    .some((v) => v?.ergebnis === "kunde");
}
...
export function rueckeVor(lead, neueArt, neuerZeitpunkt, wer = null) {
  ...
  return {
    ...
    //
    // Ausser beim Kunden. Wurde "kunde" hier gelöscht, sprang beim Planen
    // des Check-ins der Balken von 85 % zurück, und der Kunde verschwand
    // aus der Kundenliste, der Auswertung und den Zielen.
    status: "geplant",
    outcome: istKundeGeworden(lead) ? "kunde" : null,
...
export function erreichteMarken(lead) {
  ...
  if (istKundeGeworden(lead)) erreicht.add("kunde");
