...
export async function notifyOrgManagers(admin, organizationId, { subject, html, fromName, art }) {
  try {
    ...
    const { data: managers } = await admin.from("profiles").select("id, benachrichtigungen, zeitzone")
    ...
    // Je Empfänger rendern, wenn "html" eine Funktion ist: nur so kann die
    // Mail die Ortszeit der angeschriebenen Person nennen (lib/terminzeit.js).
    // Ein fester Text ginge an alle gleich.
    const empfaenger = managers
      .filter((m) => !art || willMeldung(m, art))
      .map((m) => ({ profil: m, to: emailById.get(m.id) }))
      .filter((e) => e.to);
    ...
    await Promise.all(empfaenger.map(({ to, profil }) =>
      sendEmail({ to, subject, html: typeof html === "function" ? html(profil) : html, fromName })));
  } catch (e) {
