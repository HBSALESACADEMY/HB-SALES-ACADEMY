...
export function leseAnweisung(heute) {
  return [
    ...
    ' "ergebnis": "kunde" | "ueberlegt" | "kein_abschluss" | "nicht_stattgefunden" | "stattgefunden" | null,',
...
export function vorschlagText(lead, v) {
  ...
  if (v.ergebnis) zeilen.push(`• Ergebnis: ${ERGEBNISSE[v.ergebnis].label}`);
...
export function hatEtwasZuTun(v) {
...
async function merkeVorschlag(admin, userId, daten) {
  const { error } = await admin.from("telegram_verknuepfungen")
    .update({ modus: "eintrag", modus_daten: daten, modus_seit: new Date().toISOString() }).eq("user_id", userId);
  return !error;
}

async function vergiss(admin, userId) {
  await admin.from("telegram_verknuepfungen").update({ modus: null, modus_daten: null, modus_seit: null }).eq("user_id", userId);
}
...
export async function versucheEintrag(admin, v, text, { jetzt = new Date(), ki = callAI } = {}) {
  ...
  if (!treffer.length) {
    ...
    await admin.from("telegram_verknuepfungen").update({
      modus: "eintrag",
      modus_daten: { neuerName: vorschlag.kunde, neueZeit: vorschlag.naechster?.zeitpunkt ? vorschlag.naechster : null },
      modus_seit: new Date().toISOString(),
  ...
  if (treffer.length > 1) {
    ...
    if (!(await merkeVorschlag(admin, v.user_id, { vorschlag, kandidaten: auswahl.map((l) => l.id) }))) return false;
  ...
  if (!hatEtwasZuTun(vorschlag)) {
    await sendePersoenlich(admin, v, vorschlagText(lead, vorschlag));
    return true;
  }
  if (!(await merkeVorschlag(admin, v.user_id, { vorschlag, leadId: lead.id }))) return false;
...
export async function bearbeiteEintragKnopf(admin, knopf, { jetzt = new Date() } = {}) {
  ...
  if (/^\d$/.test(wahl)) {
    ...
    if (!hatEtwasZuTun(daten.vorschlag)) {
      await vergiss(admin, v.user_id);
      await sendeAlarm(vorschlagText(lead, daten.vorschlag), knopf.chat_id);
      return { ok: true, eingetragen: false };
    }
    await merkeVorschlag(admin, v.user_id, { vorschlag: daten.vorschlag, leadId: lead.id });
