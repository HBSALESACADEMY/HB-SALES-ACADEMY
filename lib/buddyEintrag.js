...
export async function versucheEintrag(admin, v, text, { jetzt = new Date(), ki = callAI } = {}) {
  ...
  if (!treffer.length) {
    await sendePersoenlich(admin, v, `Zu „${vorschlag.kunde}“ finde ich bei deinen Terminen der letzten drei Wochen nichts. Ist der Termin schon in der Academy angelegt?${appUrl ? `\n${appUrl}/termine` : ""}`);
    return true;
  }
  if (treffer.length > 1) {
