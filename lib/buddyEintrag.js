...
/**
 * Ist das eine Bestätigung — und nennt sie nur den Zeitpunkt DIESES Termins?
 *
 * Der Satz "Setting Call mit Müller bestätigt, Freitag 14 Uhr" sagt nichts
 * über einen Folgetermin: Freitag 14 Uhr IST dieser Termin. Die KI liefert
 * die Zeitangabe trotzdem als "naechster.zeitpunkt", weil sie im Satz steht.
 *
 * Wer daraus ein Weiterrücken macht, richtet zwei Schäden an: Der Termin
 * wandert eine Stufe vor (aus dem Setting Call wird ein Closing Call), und
 * die Gruppe bekommt "🕐 Termin verschoben" gemeldet — obwohl niemand etwas
 * verschoben hat. Gemeldet werden muss "Termin bestätigt, findet am … statt".
 */
export function istNurBestaetigung(vorschlag = {}) {
  return !!ERGEBNISSE[vorschlag.ergebnis]?.bestaetigung;
}

export function eintragPatch(lead, vorschlag, userId, heute = berlinHeute()) {
  let patch = { ...(ERGEBNISSE[vorschlag.ergebnis]?.patch || {}) };
  const bestaetigung = istNurBestaetigung(vorschlag);
  if (bestaetigung) {
  ...
  if (vorschlag.naechster?.zeitpunkt && !bestaetigung) {
    patch = { ...patch, ...rueckeVor({ ...lead, ...patch }, vorschlag.naechster.art, vorschlag.naechster.zeitpunkt, userId) };
  } else if (vorschlag.naechster?.zeitpunkt && bestaetigung
      && new Date(vorschlag.naechster.zeitpunkt).getTime() !== new Date(lead.appointment_at).getTime()) {
    // Eine Bestätigung mit ANDERER Uhrzeit: Der Kunde hat zugesagt, aber zu
    // einem verschobenen Zeitpunkt. Dann gehört die neue Zeit an denselben
    // Termin — nicht an eine neue Stufe.
    patch.appointment_at = vorschlag.naechster.zeitpunkt;
  }
  if (vorschlag.notiz) {
...
export function vorschlagText(lead, v) {
  ...
  if (v.naechster?.zeitpunkt) {
    // "Nächster Termin" wäre bei einer Bestätigung eine Zusage, die niemand
    // gegeben hat: Der genannte Zeitpunkt ist dieser Termin.
    zeilen.push(istNurBestaetigung(v)
      ? `• Findet statt am ${wann(v.naechster.zeitpunkt)}`
      : `• Nächster Termin: ${stufe(v.naechster.art)} ${wann(v.naechster.zeitpunkt)}`);
  }
...
export async function bearbeiteEintragKnopf(admin, knopf, { jetzt = new Date() } = {}) {
  ...
  // Bei einer Bestätigung geht NUR die Bestätigung raus. Sonst las die
  // Gruppe "🕐 Termin verschoben" für einen Termin, der genau dann
  // stattfindet, wann er immer stattfinden sollte.
  if (vorschlag.naechster?.zeitpunkt && !istNurBestaetigung(vorschlag)) {
