...
/** Wie ein Monat heisst: "September 2026". */
export function monatsTitel(tagStr) {
  if (!tagStr) return "Ohne Zeitpunkt";
  const d = new Date(`${tagStr}T12:00:00Z`);
  return `${MONATE[d.getUTCMonth()]} ${d.getUTCFullYear()}`;
}

/**
 * Vergangene Termine nach MONATEN bündeln.
 *
 * Nach Tagen gebündelt bestand die Vergangenheitsansicht zur Hälfte aus
 * Überschriften: dreissig Termine ergaben zwanzig Tagesgruppen mit je einer
 * Karte. Was man dort sucht, ist ohnehin ein Name oder ein Zeitraum, nicht
 * "was war am Dienstag".
 */
export function gruppiereNachMonat(leads = []) {
  const gruppen = [];
  const nachSchluessel = new Map();

  leads.forEach((l) => {
    const tag = l?.appointment_at ? deutscherTag(l.appointment_at) : null;
    const schluessel = tag ? tag.slice(0, 7) : "ohne";
    if (!nachSchluessel.has(schluessel)) {
      const gruppe = { schluessel, tag, titel: monatsTitel(tag), datum: "", istHeute: false, leads: [] };
      nachSchluessel.set(schluessel, gruppe);
      gruppen.push(gruppe);
    }
    nachSchluessel.get(schluessel).leads.push(l);
