...
// extra: weitere Felder für sendMessage, etwa Knöpfe unter der Nachricht
// (reply_markup) — für die Ergebnisfrage des Vertriebsbuddys.
export async function sendeAlarm(text, zielChat, extra = {}) {
  ...
  try {
    const res = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
      ...
      body: JSON.stringify({ chat_id: chatId, text, disable_notification: false, ...extra }),
    });
    if (!res.ok) {
