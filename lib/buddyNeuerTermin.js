...
async function setzeModus(admin, userId, modus, daten = null) {
  ...
  if (error) console.error("Vertriebsbuddy: Termin-Dialog nicht gespeichert:", error.message);
  return !error;
}
...
export async function starteNeuenTermin(admin, v, profil, vorgabe = {}) {
  ...
  if (!(await setzeModus(admin, v.user_id, "neuerTermin", { werte, fragen }))) {
    return sendePersoenlich(admin, v, "Den Termin-Dialog kann ich gerade nicht starten — in der Datenbank fehlt noch eine Änderung (migration_175).");
  }
...
export async function neuerTerminAntwort(admin, v, text) {
  ...
  if (!fragen.length) { await setzeModus(admin, v.user_id, null); return false; }
  ...
  await setzeModus(admin, v.user_id, "neuerTermin", { werte: neueWerte, fragen });
