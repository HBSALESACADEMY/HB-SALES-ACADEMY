...
const VEREINBART = /\b(ausgemacht|vereinbart|steht|angesetzt|terminiert|gelegt|geplant|festgemacht|klargemacht|abgemacht)\b/i;
...
export function willStufe(text) {
  ...
  const treffer = Object.entries(ZIELE).find(([, z]) => z.worte.test(t));
...
export function auswahlText(zielKey, leads = [], nameVon = null) {
  ...
  return [`Für wen ist der ${label(ziel.art)}?`, "", ...zeilen].join("\n");
}
...
export function bestaetigungsFrage(lead, zielKey, zeit) {
  ...
  return [
    ...
    `• Neu: ${label(ziel.art)} ${deutscheZeit(zeit.zeitpunkt)} Uhr`,
  ].join("\n");
}
...
export async function bearbeiteStufenKnopf(admin, knopf) {
  ...
  const patch = rueckeVor(lead, ziel.art, daten.zeit.zeitpunkt, v.user_id);
  ...
  await sendeTerminMeldung(admin, {
    ...
    grund: "verschoben",
    ereignis: "bearbeitet",
    beschreibung: `${label(ziel.art)} mit ${lead.name} am ${deutscheZeit(daten.zeit.zeitpunkt)} Uhr.`,
