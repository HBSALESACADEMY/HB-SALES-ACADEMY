...
export async function zeigeStufenAuswahl(admin, v, profil, text, { jetzt = new Date() } = {}) {
  ...
  if (!(await setzeModus(admin, v, "stufe", { ziel: zielKey, kandidaten: auswahl.map((l) => l.id) }))) return false;
...
export async function stufenAntwort(admin, v, text) {
  ...
  await setzeModus(admin, v, "stufe", { ...daten, zeit });
...
export async function bearbeiteStufenKnopf(admin, knopf) {
  ...
  if (wahl.leadId) {
    ...
    await setzeModus(admin, v, "stufe", { ziel: daten.ziel, leadId: lead.id });
