...
import { bestaetigungsKnoepfe } from "./buddyBestaetigung.js";
import { anwahlSerie } from "./anwahlSpiel.js";
...
/**
 * Die Zeile zur Anwahl-Serie — oder null.
 *
 * Sie steht unter dem Briefing, nicht darüber: Der Tag beginnt mit den
 * Terminen, nicht mit einer Zahl, die man halten muss.
 */
export function serienZeile(serie) {
  if (!serie || serie.laenge <= 0) return null;
  if (serie.heuteGeschafft) return `🔥 Anwahl-Serie: ${serie.laenge} ${serie.laenge === 1 ? "Tag" : "Tage"} — heute schon gehalten.`;
  return `🔥 Anwahl-Serie: ${serie.laenge} ${serie.laenge === 1 ? "Tag" : "Tage"}. ${serie.mindestens} Anwahlen halten sie heute.`;
}

/** Die Nachricht mit den Terminen von heute — oder null, wenn keine anstehen. */
export function briefingText({ name = "", termine = [], appUrl = "", serie = null } = {}) {
 ...
 */
/** Die eigenen Anwahl-Tage der letzten Wochen — für die Serie. */
async function ladeAnwahlTage(admin, userId, jetzt) {
  const heute = berlinHeute(jetzt);
  const { data } = await admin.from("call_log_days")
    .select("log_date, counts").eq("user_id", userId)
    .gte("log_date", tagPlus(heute, -60)).order("log_date", { ascending: false });
  return data || [];
}
...
export async function briefingFuerPerson(admin, verknuepfung, { name = "", jetzt = new Date(), termine = null, nurHeute = false } = {}) {
  ...
  let gesendet = 0;
  // Die Serie steht unter dem Briefing: ein Grund, heute zum Hörer zu
  // greifen (lib/anwahlSpiel.js).
  let serie = null;
  try {
    serie = anwahlSerie(await ladeAnwahlTage(admin, verknuepfung.user_id, jetzt), { jetzt });
  } catch (e) {
    console.error("Anwahl-Serie nicht ladbar:", e.message);
  }
  const text = briefingText({ name, termine: heute, appUrl, serie });
  if (text) {
    // Unter dem Briefing ein Knopf je Termin, dem noch die Bestätigung
    // fehlt — der Hinweis darauf steht ja ohnehin darüber.
    const knoepfe = bestaetigungsKnoepfe(heute);
    const versand = await sendePersoenlich(admin, verknuepfung, text, knoepfe ? { reply_markup: knoepfe } : {});
