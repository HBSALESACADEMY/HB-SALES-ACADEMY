...
export const PLATZHALTER = [
  ...
  // Der Buchungslink mit Herkunft (lib/buchungslink.js): wer bucht, trägt
  // mit, von welchem Vertriebler und aus welchem Kontakt er kam.
  { schluessel: "buchungslink", label: "Link zur Terminbuchung, mit Herkunft" },
];
...
export function unbekanntePlatzhalter(text) {
  // Die Markenplatzhalter gehören dazu — sonst meldete jede HTML-Vorlage
  // mit Logo einen "unbekannten Platzhalter", der gar keiner ist.
  const erlaubt = new Set([...PLATZHALTER, ...MARKEN_PLATZHALTER].map((p) => p.schluessel));
...
/**
 * Vorname und Nachname zu einem Namen verbinden — für alles, was "name"
 * liest: die Liste, den Termin, die Meldungen.
 */
export function ganzerName(vorname, nachname) {
  return [vorname, nachname].map((t) => String(t || "").trim()).filter(Boolean).join(" ");
}

/**
 * Einen alten Namen in zwei Felder aufteilen — als Vorschlag, nicht als
 * Wahrheit. Das letzte Wort wird Nachname, der Rest Vorname. Bei "Anna von
 * der Heide" liegt das daneben; genau deshalb stehen beide Felder offen
 * zum Korrigieren da, statt dass die Aufteilung still gespeichert wird.
 */
...
export function werteFuerKontakt(kontakt = {}, { vertriebler = "", organisation = "", logo = "", farbe = "", farbe2 = "", buchungslink = "" } = {}) {
  return {
    anrede: anredeText(kontakt.anrede),
    // Der erfasste Nachname, wo es ihn gibt (migration_162). Nur für alte
    // Kontakte ohne getrenntes Feld gilt weiter das letzte Wort — das geht
    // bei "Anna von der Heide" schief, und genau deshalb gibt es das Feld.
    nachname: String(kontakt.nachname || "").trim() || nachnameAus(kontakt.name),
    name: kontakt.name || "",
    firma: kontakt.firma || "",
    notiz: kontakt.notiz || "",
    vertriebler,
    organisation,
    // Das Erscheinungsbild der Organisation (lib/marketingVorlage.js
    // markeAus). Geprüft, bevor es hier ankommt.
    logo: sichereBildAdresse(logo),
    farbe: sichereFarbe(farbe),
    farbe2: sichereFarbe(farbe2),
    // Nur eine https-Adresse ohne Anführungszeichen — der Wert steht in
    // einem href-Attribut.
    buchungslink: sichereBildAdresse(buchungslink),
  };
}

// Platzhalter für das Erscheinungsbild der Organisation.
//
// Eine HTML-Vorlage passte sich bisher nicht an: Logo und Farben wurden
// nie an sie übergeben, also blieb stehen, was fest in der Datei stand.
// Wer dieselbe Vorlage für zwei Organisationen nutzt — oder sein Logo
// ändert —, hatte in jeder Mail das alte.
export const MARKEN_PLATZHALTER = [
  { schluessel: "logo", label: "Logo der Organisation, als Bildadresse für <img src=\"{{logo}}\">" },
  { schluessel: "farbe", label: "Hauptfarbe, z. B. für style=\"background:{{farbe}}\"" },
  { schluessel: "farbe2", label: "Zweitfarbe" },
];

/**
 * Nur eine echte Farbangabe durchlassen.
 *
 * Der Wert landet in einem style-Attribut. Stünde dort etwas anderes als
 * "#abc" oder "#aabbcc", liesse sich über das Farbfeld der Organisation
 * beliebiges CSS in jede Mail schreiben.
 */
export function sichereFarbe(wert) {
  const w = String(wert || "").trim();
  return /^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(w) ? w : "";
}

/**
 * Nur eine öffentliche https-Adresse als Bild.
 *
 * Mailprogramme laden Bilder nur von dort. Ein relativer Pfad oder ein
 * eingebettetes Bild käme beim Kunden als leeres Kästchen an.
 */
export function sichereBildAdresse(wert) {
  const w = String(wert || "").trim();
  return /^https:\/\/[^\s"'<>]+$/i.test(w) ? w : "";
}

/** Die Markenwerte aus einer Organisation, in der Form, die Vorlagen brauchen. */
export function markeAus(org = {}) {
  return {
    logo: org?.logo_url || "",
    farbe: org?.primary_color || "",
    farbe2: org?.secondary_color || "",
  };
}
