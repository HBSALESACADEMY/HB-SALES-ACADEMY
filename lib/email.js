// Zentraler E-Mail-Versand über die Resend-API (resend.com), per fetch statt
// eigenem SDK — keine zusätzliche Abhängigkeit nötig. Ohne gesetzten
// RESEND_API_KEY wird der Versand übersprungen statt zu crashen, damit die
// App auch ohne konfigurierten E-Mail-Versand normal funktioniert.
// Aus der Antwort von Resend den lesbaren Teil holen. Die Rohantwort ist
// JSON und in einer Fehlermeldung auf dem Bildschirm unbrauchbar.
function kurzeMeldung(body) {
  try {
    const daten = JSON.parse(body);
    return daten?.message || daten?.error?.message || body.slice(0, 300);
  } catch (e) {
    return String(body || "").slice(0, 300);
  }
}

/**
 * Die Abmelde-Kopfzeilen einer Werbemail.
 *
 * Postfächer wie Gmail, web.de und GMX erwarten sie bei Werbung: ohne
 * "List-Unsubscribe" gilt eine Mail als Massenversand von jemandem, der
 * sich nicht an die Regeln hält, und landet eher im Spam. Mit ihr zeigt
 * das Postfach selbst einen Abmelde-Knopf — und wer sich abmelden will,
 * klickt den, statt "Spam" zu drücken. Genau dieser Klick ruiniert sonst
 * den Ruf der Absenderdomain.
 *
 * Als mailto-Adresse, weil die Academy dafür keine eigene Abmeldeseite
 * braucht: die Mail kommt bei der Antwortadresse der Organisation an.
 *
 * NUR für echten Massenversand. Die Marketing-Mails der Academy setzen sie
 * nicht: Mailprogramme zeigen damit "Diese Nachricht stammt von einer
 * Mailingliste" an, und eine persönliche Mail nach einem Telefonat liest
 * sich dann wie eine Rundmail (pages/api/marketing-mail.js).
 */
export function abmeldeKopfzeilen(adresse) {
  const a = String(adresse || "").trim();
  if (!/^[^@\s<>]+@[^@\s<>]+\.[^@\s<>]+$/.test(a)) return null;
  return { "List-Unsubscribe": `<mailto:${a}?subject=Abmelden>` };
}

export async function sendEmail({ to, subject, html, text, fromName, fromEmail, replyTo, attachments, abmeldung }) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) {
    console.log("E-Mail-Versand übersprungen (RESEND_API_KEY nicht gesetzt):", subject, "an", to);
    return { skipped: true };
  }
  // RESEND_FROM_EMAIL darf entweder nur die Adresse ("noreply@domain.de")
  // oder "Name <adresse>" enthalten — der Name-Teil wird durch fromName
  // (z.B. den Organisationsnamen) ersetzt, falls übergeben.
  const configuredFrom = process.env.RESEND_FROM_EMAIL || "HB Sales Academy <onboarding@resend.dev>";
  // Eine eigene Adresse der Organisation hat Vorrang (migration_142). Sie
  // muss bei Resend verifiziert sein — steht dort eine fremde Domain, lehnt
  // Resend die Mail ab, und zwar für ALLE Mails dieser Organisation.
  const fromAddress = (fromEmail && fromEmail.includes("@"))
    ? fromEmail.trim()
    : (configuredFrom.match(/<(.+)>/)?.[1] || configuredFrom);
  const from = fromName ? `${fromName} <${fromAddress}>` : configuredFrom;
  try {
    const res = await fetch("https://api.resend.com/emails", {
      method: "POST",
      headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
      body: JSON.stringify({
        from,
        to: Array.isArray(to) ? to : [to],
        subject,
        html,
        // Die Textfassung derselben Mail, wenn es eine gibt. Mailprogramme
        // ohne HTML zeigen sie an, und Spamfilter werten eine HTML-Mail
        // ohne Textfassung als Zeichen von Massenversand.
        ...(text ? { text } : {}),
        // Nur bei Werbemails (abmeldung gesetzt) — eine Benachrichtigung
        // "Neue Aufgabe" an das eigene Team hat keinen Abmeldeknopf.
        ...(abmeldeKopfzeilen(abmeldung) ? { headers: abmeldeKopfzeilen(abmeldung) } : {}),
        // Antworten landen dort, wo sie hingehören — auch wenn der Absender
        // aus technischen Gründen eine andere Adresse tragen muss.
        ...(replyTo && replyTo.includes("@") ? { reply_to: replyTo.trim() } : {}),
        // Anhänge als Base64 — Resend erwartet {filename, content}. Ohne
        // Dateien bleibt das Feld weg statt leer dazustehen.
        ...(Array.isArray(attachments) && attachments.length ? { attachments } : {}),
      }),
    });
    if (!res.ok) {
      const body = await res.text();
      console.error("E-Mail-Versand fehlgeschlagen: " + JSON.stringify({ to, status: res.status, body }));
      // Der Text von Resend wird mitgegeben, nicht nur ein Fehler-Flag: bei
      // einer nicht verifizierten Domain steht dort genau, was fehlt — und
      // ohne diese Auskunft sucht man an der falschen Stelle.
      return { error: true, status: res.status, meldung: kurzeMeldung(body), absender: from };
    }
    // Die Kennung mitgeben: nur mit ihr lässt sich eine spätere
    // Rückmeldung ("unzustellbar") dem richtigen Kontakt zuordnen. Über die
    // Adresse allein ginge es schief, sobald zwei Mails an dieselbe Adresse
    // gingen.
    let kennung = null;
    try { kennung = (await res.json())?.id || null; } catch (e) { /* ohne Kennung weiter */ }
    return { ok: true, absender: from, id: kennung };
  } catch (e) {
    console.error("E-Mail-Versand fehlgeschlagen:", e.message);
    return { error: true, meldung: e.message, absender: from };
  }
}
