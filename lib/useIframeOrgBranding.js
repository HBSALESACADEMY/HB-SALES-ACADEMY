...
export function useIframeOrgBranding() {
  ...
  const [leadFieldConfig, setLeadFieldConfig] = useState(null);
    ...
    (async () => {
      ...
      if (cached) {
        if (mounted) {
          ...
          setLeadFieldConfig(Array.isArray(cached.lead_field_config) && cached.lead_field_config.length ? cached.lead_field_config : null);
          setLoaded(true);
        }
        return;
      }
      ...
      if (activeOrgId) {
        ...
        if (mounted && org) {
          ...
          setLeadFieldConfig(Array.isArray(org.lead_field_config) && org.lead_field_config.length ? org.lead_field_config : null);
        }
      }
  ...
  return { logoUrl, orgName, colors, bookingInstructions, objectionCategories, leadFieldConfig, loaded };
}
