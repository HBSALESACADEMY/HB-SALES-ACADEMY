...
export async function callAI(system, messages, maxTokens = 300) {
  ...
  // 429 = eigenes Freikontingent kurz überschritten. 503 = Google-Server gerade
  // überlastet (kommt gelegentlich vor, meist nur für Sekunden). Beides lohnt
  // sich, mit steigender Wartezeit erneut zu versuchen, bevor wir aufgeben.
  let attempt = 0;
  const backoffs = [3000, 6000, 10000];
  while ((res.status === 429 || res.status === 503) && attempt < backoffs.length) {
