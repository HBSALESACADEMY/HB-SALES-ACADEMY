...
export async function spracheErzeugen(text, stimme = "Kore", art = "") {
  if (!process.env.GEMINI_API_KEY || !text) return null;
  // Das TTS-Modell nimmt eine Regieanweisung VOR dem Text entgegen. Ohne sie
  // klingt jede Person gleich — nach Vorleser statt nach Gesprächspartner.
  const gesprochen = art ? `Sprich ${art}. Sage genau das Folgende auf Deutsch: ${text}` : text;
  try {
    await waitForFreeSlot();
    const res = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${TTS_MODELL}:generateContent?key=${process.env.GEMINI_API_KEY}`,
      {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          contents: [{ parts: [{ text: gesprochen }] }],
          generationConfig: {
