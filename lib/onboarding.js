...
/** Ganze Tage zwischen zwei Kalendertagen ("2026-09-10", "2026-09-14") → 4. */
...
/**
 * Der Blick aufs Ganze: Wie steht die Organisation im Onboarding?
 *
 * "engpaesse" ist der eigentliche Wert: der Schritt, an dem die meisten
 * hängen, sagt mehr als jeder einzelne Balken. Steht dort immer dasselbe,
 * liegt es selten an den Leuten, sondern am Schritt.
 */
export function onboardingUebersicht(zuweisungen = []) {
  const laufend = zuweisungen.filter((z) => !z.stand?.fertig);
  const fertige = zuweisungen.filter((z) => z.stand?.fertig);

  const dauern = fertige
    .map((z) => (z.abgeschlossen_am ? tageZwischen(z.gestartet_am, datumTag(z.abgeschlossen_am)) : null))
    .filter((n) => n !== null && n >= 0);

  const offenJeSchritt = new Map();
  laufend.forEach((z) => (z.stand?.liste || []).forEach((x) => {
    if (x.erledigt) return;
    const e = offenJeSchritt.get(x.schritt.id) || { id: x.schritt.id, titel: x.schritt.titel, offen: 0, ueberfaellig: 0 };
    e.offen += 1;
    if (x.ueberfaellig) e.ueberfaellig += 1;
    offenJeSchritt.set(x.schritt.id, e);
  }));

  return {
    laufend: laufend.length,
    fertig: fertige.length,
    prozent: laufend.length ? Math.round(laufend.reduce((s, z) => s + z.stand.prozent, 0) / laufend.length) : 0,
    ueberfaellig: laufend.reduce((s, z) => s + (z.stand?.ueberfaellig || 0), 0),
    mitRueckstand: laufend.filter((z) => z.stand?.ueberfaellig > 0).length,
    dauerSchnitt: dauern.length ? Math.round(dauern.reduce((a, b) => a + b, 0) / dauern.length) : null,
    engpaesse: [...offenJeSchritt.values()]
      .sort((a, b) => b.ueberfaellig - a.ueberfaellig || b.offen - a.offen)
      .slice(0, 5),
  };
}

/** Zeitpunkt → Kalendertag in Berlin ("2026-09-14"). */
function datumTag(zeitpunkt) {
