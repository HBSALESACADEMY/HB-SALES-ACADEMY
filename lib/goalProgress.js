...
export async function werteProPerson(admin, metrik, ids, startISO, startTag, endeISO = null, endeTag = null) {
  const werte = new Map();
  if (!metrik || !ids.length) return werte;
  const addiere = (id, n) => { if (n) werte.set(id, (werte.get(id) || 0) + n); };
  // Obergrenze nur setzen, wenn eine da ist — laufende Zeiträume enden erst
  // in der Zukunft, dann wäre jede Schranke überflüssige Arbeit.
  const bis = (q, spalte, wert) => (wert ? q.lt(spalte, wert) : q);
  const bisTag = (q, spalte, wert) => (wert ? q.lte(spalte, wert) : q);

  if (metrik.quelle === "zeilen") {
    const { data } = await bis(admin.from(metrik.tabelle).select("user_id").in("user_id", ids).gte("created_at", startISO), "created_at", endeISO);
    ...
    const { data } = await bisTag(admin.from("call_log_days").select("user_id, counts").in("user_id", ids).gte("log_date", startTag), "log_date", endeTag);
    (data || []).forEach((z) => addiere(z.user_id, Number(z.counts?.[metrik.feld]) || 0));
  } else if (metrik.quelle === "leads") {
    // Manche Kennzahlen zählen nicht alle Termine, sondern nur solche mit
    // einem bestimmten Ergebnis (Kunde geworden, Absage) oder Status.
    let q = admin.from("leads").select("created_by").is("geloescht_am", null).in("created_by", ids).gte("created_at", startISO);
    if (metrik.filterSpalte) q = q.eq(metrik.filterSpalte, metrik.filterWert);
    const { data } = await bis(q, "created_at", endeISO);
    (data || []).forEach((z) => addiere(z.created_by, 1));
  } else if (metrik.quelle === "xp") {
    const { data } = await bis(admin.from("xp_log").select("user_id, amount").in("user_id", ids).gt("created_at", startISO), "created_at", endeISO);
