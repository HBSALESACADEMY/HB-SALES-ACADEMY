...
import { teamStand } from "./teamlageVersand.js";
import { teamZeilenFuerKI, leitungsAnweisung } from "./teamlage.js";
import { klingtNachEinwand, ladeEinwaende, passendeEinwaende, einwandZeilen } from "./buddyEinwand.js";
import { imRollenspiel, rollenspielZug, starteRollenspiel, gespraechVorbereiten, fuehreBefehlAus } from "./buddyBefehle.js";
import { istAbgelaufen } from "./buddyRollenspiel.js";
import { sendeAlarm } from "./alarm.js";
import { nameAusFrage } from "./buddyVorbereitung.js";
import { versucheEintrag, klingtNachErgebnis } from "./buddyEintrag.js";
import { willNeuenTermin, starteNeuenTermin, neuerTerminAntwort } from "./buddyNeuerTermin.js";
import { willBestaetigen, willHaken, zeigeBestaetigungsAuswahl, zeigeHakenAuswahl } from "./buddyBestaetigung.js";
import { willStufe, zeigeStufenAuswahl, stufenAntwort } from "./buddyStufe.js";
...
async function personen(admin, ids) {
  const { data: profile } = await admin.from("profiles")
    .select("id, full_name, organization_id, role, is_admin, is_platform_admin").in("id", ids);
...
// Der Buddy im freien Gespräch kann nichts in der Academy ändern. Ohne
// diese Regel antwortet die KI auf "Termin mit Müller: Kunde geworden!"
// gern mit "Super, hab ich eingetragen" — und der Vertriebler verlässt
// sich darauf, während in der Academy nichts steht.
// Geht es um einen bestimmten Termin, bleibt der Buddy dabei.
export const BEIM_TERMIN_BLEIBEN = [
  "Es geht gerade um einen bestimmten Termin. Bleib dabei:",
  "Antworte nur zu diesem Termin und zum nächsten Schritt.",
  "Keine Frage nach der Woche, keine Nachfrage zur Übung, kein anderes Thema — auch nicht am Ende als Anhängsel.",
];

// Eine klare Ansage zu einem Termin — im Unterschied zu einem Bericht über
// ein Gespräch ("der Closing Call lief gut").
const ERGEBNIS_WORTE = /\b(kunde geworden|ist kunde|kein abschluss|nicht erschienen|abgesagt|überlegt noch|will überlegen)\b/i;

export function terminAktion(text = "") {
  return !!(willStufe(text) || willBestaetigen(text) || willHaken(text) || willNeuenTermin(text) || ERGEBNIS_WORTE.test(text));
}

/**
 * Was zu tun ist, wenn eine klare Termin-Ansage nirgends gegriffen hat.
 *
 * Fester Text statt KI: Genau hier hat der Buddy bisher frei formuliert —
 * und dabei behauptet, er habe etwas "zur Bestätigung hinterlegt". Wenn
 * nichts passiert ist, muss genau das dastehen.
 */
export function terminHinweisText(appUrl = process.env.NEXT_PUBLIC_APP_URL || "") {
  return [
    "Dazu habe ich keinen passenden Termin gefunden — eingetragen habe ich nichts.",
    "",
    "So findet die Academy ihn:",
    "• Ergebnis: „Müller: Kunde geworden“",
    "• Nächste Stufe: „Closing Call ausgemacht“",
    "• Bestätigung: „Setting Call bestätigen“",
    "• Verschieben: „Termin mit Müller verschoben“",
    "• Neuer Kontakt: „neuer Termin“ oder /termin",
    "",
    "Danach zeige ich dir die passenden Kontakte, und nach deinem Tippen steht es in der Academy.",
    appUrl ? `\n${appUrl}/termine` : null,
  ].filter((z) => z !== null).join("\n");
}

/** Dreht sich die Nachricht um einen konkreten Termin? */
export function terminThema(text = "") {
  return !!(klingtNachErgebnis(text) || willStufe(text) || willBestaetigen(text) || willNeuenTermin(text));
}

export const NICHTS_EINGETRAGEN = [
  "Zum Eintragen in die Academy: Du selbst schreibst im Gespräch nichts hinein. Die Academy trägt ein,",
  "sobald die Person einen Vorschlag mit „Ja“ bestätigt — dafür braucht es die richtigen Worte von ihr.",
  "Behaupte deshalb nie, etwas eingetragen, gespeichert, notiert, hinterlegt, übermittelt, weitergegeben,",
  "vorgemerkt oder zur Bestätigung eingereicht zu haben. Sag aber auch NICHT, dass in der Academy nichts",
  "eingetragen werden könne — das ist falsch, und es hält die Person von dem ab, was sie gerade will.",
  "Sag stattdessen, wie es geht, und nimm die Worte aus der Nachricht auf:",
  "- Ergebnis: \"Müller: Kunde geworden\", \"kein Abschluss\", \"überlegt noch\", \"nicht erschienen\"",
  "- Nächste Stufe: \"Closing Call ausgemacht\" — dann kommen die passenden Kontakte zur Auswahl",
  "- Bestätigung: \"Setting Call bestätigen\"   - Verschieben: \"Termin mit Müller verschoben\"",
  "- Neuer Kontakt: \"neuer Termin\" oder /termin   - Der Tag: /termine",
  "Danach kommt jedes Mal ein Vorschlag mit Knöpfen, und nach einem Tippen steht es in der Academy.",
];
...
async function antworte(admin, verknuepfung, profil, woche, eingang = "") {
  ...
  // Geht es gerade um einen konkreten Termin, bleibt die Übung aussen vor.
  // Sonst hängt hinter der Antwort zum Closing Call noch eine Frage nach
  // den Follow-ups der Woche — und das liest sich wie ein Themenwechsel
  // mitten im Satz.
  const umEinenTermin = terminThema(eingang);
  const schulungsZeilen = baustein && !umEinenTermin
    ? [
      "",
  ...
  // Die Leitung fragt den Buddy auch nach ihren Leuten. Dafür bekommt er
  // die Zahlen und Stichpunkte des Teams — nie deren Gespräche.
  let teamZeilen = [];
  const istLeitung = istFuehrungsrolle(profil);
  if (istLeitung && profil?.organization_id) {
    try {
      const stand = await teamStand(admin, profil.organization_id);
      teamZeilen = [...leitungsAnweisung(), "", ...teamZeilenFuerKI(stand.personen, stand)];
    } catch (e) {
      console.error("Vertriebsbuddy: Teamstand nicht ladbar:", e.message);
    }
  }

  // Klingt die Nachricht nach einem Einwand, bekommt der Buddy die
  // passenden Antworten aus dem Leitfaden der eigenen Organisation — damit
  // er nicht eine eigene Einwandbehandlung erfindet.
  let einwandHilfe = [];
  if (eingang && klingtNachEinwand(eingang)) {
    try {
      const treffer = passendeEinwaende(await ladeEinwaende(admin, profil?.organization_id), eingang, 2);
      if (treffer.length) {
        einwandHilfe = [
          "",
          "Die Person schildert einen Einwand. So steht er im Leitfaden der Firma — nimm das als Grundlage, erfinde keine eigene Methode und gib einen Satz, den man am Telefon wörtlich sagen kann:",
          ...einwandZeilen(treffer),
        ];
      }
    } catch (e) {
      console.error("Vertriebsbuddy: Einwände nicht ladbar:", e.message);
    }
  }
  ...
  const antwort = await kiSatz(
    [
      istLeitung
        ? `Du bist der Vertriebsbuddy der HB Sales Academy und sprichst mit ${profil?.full_name || "der Leitung"}${profil?.organisation ? ` von ${profil.organisation}` : ""}, auf Deutsch und per Du.`
      ...
      ...schulungsZeilen,
      ...(teamZeilen.length ? ["", ...teamZeilen] : []),
      ...einwandHilfe,
      ...(umEinenTermin ? ["", ...BEIM_TERMIN_BLEIBEN] : []),
      "",
      ...NICHTS_EINGETRAGEN,
    ].join("\n"),
    nachrichten,
  ...
  const text = antwort
    || "Danke dir, deine Nachricht ist angekommen. Gerade komme ich an meine Antworten nicht heran, schreib mir später noch mal.";
...
export async function beantworteEingang(admin, eingang = {}) {
  ...
  const v = await verknuepfungZumChat(admin, chatId);
  if (!v) return { ignoriert: true };
  const profil = (await personen(admin, [v.user_id])).get(v.user_id);

  // Erst die Übungen: Im Rollenspiel ist jede Nachricht ein Satz an den
  // "Kunden" und gehört nicht in den Gesprächsverlauf des Buddys.
  if (imRollenspiel(v)) {
    await rollenspielZug(admin, v, text);
    return { ok: true, rollenspiel: true };
  }
  // Ein laufender Termin-Dialog: Die Nachricht ist die Antwort auf die
  // letzte Frage (lib/buddyNeuerTermin.js).
  if (v.modus === "neuerTermin" && !istAbgelaufen(v.modus_seit)) {
    await neuerTerminAntwort(admin, v, text);
    return { ok: true, neuerTermin: true };
  }
  // Ein laufender Stufen-Dialog wartet auf Datum und Uhrzeit.
  if (v.modus === "stufe" && !istAbgelaufen(v.modus_seit) && await stufenAntwort(admin, v, text)) {
    return { ok: true, stufe: true };
  }
  if (v.modus) await admin.from("telegram_verknuepfungen").update({ modus: null, modus_daten: null, modus_seit: null }).eq("user_id", v.user_id);

  if (willNeuenTermin(text)) {
    await starteNeuenTermin(admin, v, profil);
    return { ok: true, neuerTermin: true };
  }
  // "Ich will einen Setting Call bestätigen" — dann kommen die eigenen
  // Termine zur Auswahl, statt dass jemand einen Namen tippen muss.
  if (willBestaetigen(text) && await zeigeBestaetigungsAuswahl(admin, v, profil, text)) {
    return { ok: true, bestaetigung: true };
  }
  // Und die übrigen Haken am Termin: Check-in erledigt, Projektumsetzung.
  const haken = willHaken(text);
  if (haken && await zeigeHakenAuswahl(admin, v, profil, text, { schrittKey: haken })) {
    return { ok: true, haken };
  }
  if (/\brollenspiel\b/i.test(text) && text.length <= 160) {
    await starteRollenspiel(admin, v, text);
    return { ok: true, rollenspiel: true };
  }
  const name = istFuehrungsrolle(profil) ? nameAusFrage(text) : null;
  if (name && (await gespraechVorbereiten(admin, v, profil, name, { still: true })) !== false) {
    return { ok: true, vorbereitung: true };
  }

  // "Müller: Kunde geworden" — ein Vorschlag zum Eintragen, den die Person
  // bestätigt (lib/buddyEintrag.js). Auch das gehört nicht in den Verlauf.
  if (await versucheEintrag(admin, v, text)) return { ok: true, eintrag: true };

  // "Closing Call ausgemacht" ohne Namen: Dann kommen die Termine zur
  // Auswahl, für die dieser Schritt der nächste wäre (lib/buddyStufe.js).
  if (willStufe(text) && await zeigeStufenAuswahl(admin, v, profil, text)) {
    return { ok: true, stufe: true };
  }

  // Eine klare Ansage zu einem Termin, bei der kein Dialog gegriffen hat:
  // Dann sagt der Buddy fest, was nicht ging und wie es geht — statt sich
  // eine Antwort auszudenken.
  if (terminAktion(text)) {
    await sendePersoenlich(admin, v, terminHinweisText());
    return { ok: true, hinweis: true };
  }

  // Wer den Buddy abgeschaltet hat, bekommt keine Unterhaltung — die
  // Kurzbefehle und Übungen oben gehen trotzdem.
  if (v.buddy === false) return { ignoriert: true };
...
/**
 * Die Verknüpfung zu einem Chat — mit dem Stand einer laufenden Übung.
 *
 * Ohne migration_175 gibt es die Spalten für die Übung nicht. Dann eben
 * ohne: Gespräch und Kurzbefehle gehen trotzdem.
 */
export async function verknuepfungZumChat(admin, chatId) {
  const voll = await admin.from("telegram_verknuepfungen")
    .select("user_id, chat_id, buddy, modus, modus_daten, modus_seit").eq("chat_id", String(chatId)).limit(1);
  if (!voll.error) return voll.data?.[0] || null;
  const { data } = await admin.from("telegram_verknuepfungen")
    .select("user_id, chat_id, buddy").eq("chat_id", String(chatId)).limit(1);
  return data?.[0] || null;
}

/** Ein Kurzbefehl aus dem Chat. */
export async function beantworteBefehl(admin, eingang, befehl) {
  const v = await verknuepfungZumChat(admin, eingang.chat_id);
  if (!v) {
    await sendeAlarm("Dieser Chat ist noch nicht mit der Academy verbunden. Das geht in der Academy unter Einstellungen → Telegram.", eingang.chat_id);
    return { ignoriert: true };
  }
  const profil = (await personen(admin, [v.user_id])).get(v.user_id);
  await fuehreBefehlAus(admin, v, profil, befehl);
  return { ok: true, befehl: befehl.befehl };
}
