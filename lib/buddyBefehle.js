...
export const BEFEHLE = [
  ...
  { befehl: "termin", kurz: "Neuen Termin anlegen (ich frage alles ab)" },
...
// "termin" legt einen an, "termine" zeigt den Tag — deshalb führt hier kein
// Kurzwort vom einen zum anderen.
const ALIASE = {
  start: "hilfe", help: "hilfe", stop: "stopp", ende: "stopp", gespräch: "gespraech", ziele: "ziel",
  neu: "termin", termin_neu: "termin", anlegen: "termin",
};
...
export async function starteRollenspiel(admin, v, text = "") {
  ...
  const ok = await setzeModus(admin, v, "rollenspiel", {
    szenario: szenario.key, runden: 0, verlauf: [{ von: "kunde", text: szenario.start }],
  });
  // Beim Fehlschlag steht der Grund schon im Chat (lib/buddyModus.js).
  if (!ok) return false;
...
async function beendeRollenspiel(admin, v) {
  ...
  await setzeModus(admin, v, null, { zuletzt: daten.szenario || null });
...
export async function rollenspielZug(admin, v, text) {
  const daten = v.modus_daten || {};
  const szenario = szenarioVon(daten.szenario);
  if (!szenario) { await setzeModus(admin, v, null); return false; }
  ...
  await setzeModus(admin, v, "rollenspiel", { ...daten, runden, verlauf });
...
export async function raeumeRollenspieleAuf(admin, jetzt = new Date()) {
  ...
  const { error } = await admin.from("telegram_verknuepfungen")
    ...
    .in("modus", ["rollenspiel", "eintrag", "neuerTermin", "stufe"]).lt("modus_seit", grenze);
...
export async function fuehreBefehlAus(admin, v, profil, { befehl, rest }) {
  ...
  if (imRollenspiel(v) && befehl !== "stopp") await setzeModus(admin, v, null);
