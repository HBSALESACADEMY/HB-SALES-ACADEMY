 ...
 */
export function baueIcsFeed(termine = [], { name = "HB Sales Academy", erinnerungMinuten = 30 } = {}) {
  const jetzt = icsZeitpunkt(new Date());
  const kopf = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
  ...
  termine.forEach((t) => {
    ...
    // Eine Erinnerung im Kalender selbst. Ein Abo-Kalender wird abgeholt,
    // nicht zugestellt — die Academy kann also nichts auf ein Telefon
    // schicken. Was sie kann: den Alarm gleich mitliefern, damit das
    // Telefon ihn selbst stellt.
    //
    // Nur bei Terminen mit Uhrzeit und nur, wenn sie nicht abgesagt sind:
    // ein Wecker für einen ganztägigen Eintrag klingelt um Mitternacht, und
    // ein Wecker für einen abgesagten Termin ist schlicht falsch.
    if (t.start && !t.abgesagt && erinnerungMinuten > 0) {
      zeilen.push(
        "BEGIN:VALARM",
        "ACTION:DISPLAY",
        `DESCRIPTION:${maskiere(t.titel || "Termin")}`,
        `TRIGGER:-PT${Math.round(erinnerungMinuten)}M`,
        "END:VALARM"
      );
    }
