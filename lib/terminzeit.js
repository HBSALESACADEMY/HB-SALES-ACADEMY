...
// Zu welchem KALENDERTAG in Deutschland ein Zeitpunkt gehört ("JJJJ-MM-TT").
//
// Nötig überall dort, wo nach Tagen gruppiert wird: das Gerät kann in einer
// anderen Zeitzone stehen, und dann rutscht ein Abendereignis auf den
// Vortag — im Kalender wie in den Aktivitäten.
export function deutscherTag(iso) {
  if (!iso) return null;
  const d = iso instanceof Date ? iso : new Date(iso);
