...
export async function sendeErklaerungen(admin, { nurFuer = null, erzwingen = false, hoechstens = 200 } = {}) {
  ...
  const offen = (data || []).filter((v) => erzwingen || !v.erklaerung_am);
  if (!offen.length) return { gesendet: 0 };

  const { data: profile } = await admin.from("profiles")
    .select("id, role, is_admin, is_platform_admin").in("id", offen.map((v) => v.user_id));
