...
/**
 * Zwei Zeiträume als Kurven übereinander.
 *
 * Die Balkenpaare daneben beantworten "wie viel insgesamt". Sie sagen
 * nichts darüber, WANN es passiert ist: Zwei Wochen mit derselben Summe
 * können völlig verschieden verlaufen sein — die eine gleichmässig, die
 * andere mit zwei starken Tagen und drei toten. Genau das ist die Frage,
 * bei der man etwas ändern kann.
 *
 * Gelegt wird Tag über Tag: Tag 1 des früheren Zeitraums auf Tag 1 des
 * jetzigen. Die Kalenderdaten unterscheiden sich naturgemäss; auf der
 * Achse steht deshalb der jetzige Zeitraum.
 *
 * Sind die Zeiträume unterschiedlich lang, werden nur die gemeinsamen
 * ersten Tage gezeichnet. Die Alternative wäre, den kürzeren mit Nullen
 * aufzufüllen — das sähe wie "an diesen Tagen wurde nicht telefoniert"
 * aus, und das wäre eine Behauptung über Tage, die es nicht gibt.
 *
 * @param jetztWerte  [{ tag, wert }] des jetzigen Zeitraums (lib/kurve.js).
 * @param davorWerte  [{ tag, wert }] des früheren.
 * @returns { reihen, tage, gekuerzt } — "gekuerzt" sagt, ob etwas wegfiel.
 */
export function vergleichsKurven(jetztWerte = [], davorWerte = [], {
  name = "Davor", farbeJetzt = "#2FA890", farbeDavor = "#7C869C",
} = {}) {
  const jetzt = Array.isArray(jetztWerte) ? jetztWerte : [];
  const davor = Array.isArray(davorWerte) ? davorWerte : [];
  if (!jetzt.length) return { reihen: [], tage: 0, gekuerzt: false };
  if (!davor.length) {
    return {
      reihen: [{ label: "Dieser Zeitraum", farbe: farbeJetzt, werte: jetzt }],
      tage: jetzt.length,
      gekuerzt: false,
    };
  }
  const tage = Math.min(jetzt.length, davor.length);
  return {
    reihen: [
      { label: "Dieser Zeitraum", farbe: farbeJetzt, werte: jetzt.slice(0, tage) },
      { label: name, farbe: farbeDavor, werte: davor.slice(0, tage) },
    ],
    tage,
    gekuerzt: jetzt.length !== davor.length,
  };
}
