...
/** Die volle Stunde in Berlin (0–23) — für Läufe, die nach deutscher Uhrzeit gehen. */
export function berlinStunde(jetzt = new Date()) {
  // Über formatToParts statt format(): die deutsche Schreibweise hängt " Uhr"
  // an ("09 Uhr"), daraus liesse sich keine Zahl lesen.
  const teile = new Intl.DateTimeFormat("de-DE", { timeZone: ZONE, hour: "numeric", hour12: false }).formatToParts(jetzt);
...
export function wochenStartZeitpunkt(jetzt = new Date()) {
  return tagesBeginnZeitpunkt(wochenStartTag(jetzt));
}

// Der echte Zeitpunkt, an dem in Berlin ein bestimmter Tag beginnt, als
// ISO-Zeichenkette — für Vergleiche mit created_at, das UTC ist.
export function tagesBeginnZeitpunkt(tagStr) {
  const probe = new Date(`${tagStr}T00:00:00Z`);
  return new Date(probe.getTime() - versatzMs(probe)).toISOString();
}

// Ein Datum plus Uhrzeit, beides als Berliner Zeit gemeint, als echter
// Zeitpunkt (ISO/UTC). Eine im Kalender eingetragene Uhrzeit ist immer die
// deutsche — wer sie auf einem Gerät im Ausland ausliest, darf sie nicht
// als Ortszeit verstehen, sonst wandert der Termin um Stunden.
export function zeitpunktInBerlin(tagStr, uhrzeit) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(tagStr || ""))) return null;
  const treffer = String(uhrzeit || "").match(/^(\d{1,2}):(\d{2})$/);
  if (!treffer) return null;
  const stunde = Number(treffer[1]), minute = Number(treffer[2]);
  if (stunde > 23 || minute > 59) return null;
  const probe = new Date(`${tagStr}T${String(stunde).padStart(2, "0")}:${treffer[2]}:00Z`);
  return new Date(probe.getTime() - versatzMs(probe)).toISOString();
}

// Verschiebt ein Datum "JJJJ-MM-TT" um n Tage — über UTC-Bausteine, damit
// die Ortszeit des ausführenden Rechners keine Rolle spielt.
export function tagPlus(tagStr, n) {
  const d = new Date(`${tagStr}T00:00:00Z`);
