...
/**
 * Die Tagesrangliste mit dem LAUFENDEN eigenen Zähler überschreiben.
 *
 * Vom Server kommt der Stand, der dort gespeichert ist — der ist schon beim
 * nächsten Klick veraltet. Vorher stand in der Liste "Du 21", während der
 * Zähler oben längst 25 zeigte; das sah nach einem Fehler aus.
 *
 * Genommen wird das Maximum aus beidem: Ein Abgleich, der gerade unterwegs
 * ist, soll die Zahl nicht kurz zurückspringen lassen. Danach wird neu
 * sortiert — in derselben Reihenfolge wie auf dem Server
 * (pages/api/tagesrangliste.js), sonst passt der Platz nicht mehr zum Wert.
 */
export function ranglisteMitEigenem(liste, counts = {}) {
  if (!Array.isArray(liste)) return liste ?? null;
  return [...liste]
    .map((p) => (p?.ich
      ? {
        ...p,
        anwahlen: Math.max(Number(p.anwahlen) || 0, Number(counts.anwahlen) || 0),
        termin: Math.max(Number(p.termin) || 0, Number(counts.termin) || 0),
      }
      : p))
    .sort((a, b) => (Number(b.anwahlen) || 0) - (Number(a.anwahlen) || 0)
      || (Number(b.termin) || 0) - (Number(a.termin) || 0));
}
...
// Grenzen für eine selbst eingestellte Blocklänge. Unter einer Minute ist
// kein Block, über drei Stunden auch nicht — und ein Vertipper wie 250
// statt 25 soll nicht stillschweigend durchgehen.
export const BLOCK_MIN = 1;
export const BLOCK_MAX = 180;

/** Eine eingegebene Blocklänge prüfen: { minuten } oder { fehler }. */
export function leseBlockEingabe(text) {
  const roh = String(text ?? "").trim().replace(",", ".");
  if (!roh) return { minuten: null };
  if (!/^\d{1,3}$/.test(roh)) return { fehler: "Bitte ganze Minuten eingeben, zum Beispiel 30." };
  const minuten = Number(roh);
  if (minuten < BLOCK_MIN || minuten > BLOCK_MAX) return { fehler: `Zwischen ${BLOCK_MIN} und ${BLOCK_MAX} Minuten.` };
  return { minuten };
}

// Die Uhr läuft HOCH, nicht runter.
//
// Ein Countdown macht Druck: Man sieht, wie die Zeit weggeht, und die
// letzten Minuten telefoniert man gegen die Uhr statt mit dem Menschen am
// anderen Ende. Hochzählen dreht das um — man sieht, was man schon
// gemacht hat. Die gewählten Minuten sind dann ein Ziel, keine Frist: Wer
// drüber ist, darf weiterlaufen.
//
// Damit ein vergessener Block nicht stundenlang weiterzählt und am Ende
// ein unsinniges Ergebnis liefert, hört er beim Doppelten von selbst auf.
export const BLOCK_GRENZE_FAKTOR = 2;

/** Sekunden als mm:ss — auch über eine Stunde hinaus (dann hh:mm:ss). */
export function uhrZeit(sekunden) {
  const s = Math.max(0, Math.round(Number(sekunden) || 0));
  const zwei = (n) => String(n).padStart(2, "0");
  const stunden = Math.floor(s / 3600);
  const rest = s % 3600;
  const mm = zwei(Math.floor(rest / 60));
  const ss = zwei(rest % 60);
  return stunden ? `${stunden}:${mm}:${ss}` : `${mm}:${ss}`;
}

/**
 * Wo ein laufender Block gerade steht.
 *
 * "minuten" ist das, was am Ende im Ergebnis zählt: die tatsächlich
 * gelaufene Zeit, mindestens eine Minute — nicht die geplante.
 */
export function blockStand({ minuten = 25, sekunden = 0 } = {}) {
  const zielSekunden = Math.max(1, Number(minuten) || 1) * 60;
  const gelaufen = Math.max(0, Math.round(Number(sekunden) || 0));
  const grenze = zielSekunden * BLOCK_GRENZE_FAKTOR;
  return {
    uhr: uhrZeit(gelaufen),
    zielSekunden,
    zielVoll: gelaufen >= zielSekunden,
    ueberMinuten: Math.floor(Math.max(0, gelaufen - zielSekunden) / 60),
    vorbei: gelaufen >= grenze,
    minuten: Math.max(1, Math.round(gelaufen / 60)),
  };
}
