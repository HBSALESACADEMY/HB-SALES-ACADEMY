...
export const FIELDS = [
  ...
  // "Schicken Sie mir was per Mail" ist weder Termin noch Absage, sondern
  // ein offener Faden. Als Ablehnungsgrund gezählt, gälte der Anruf in jeder
  // Statistik als verloren — dabei ist ein Kontakt entstanden
  // (migration_138).
  { key: "email", label: "E-Mail gewünscht", kind: "positive" },
...
export const OFFENE_SCHRITTE = ["outcome", "wen", "durchgestellt", "leitfaden", "callResult", "reason", "booking", "leadForm", "emailForm", "mailForm"];
...
export function loadDay(prefix, key, reasons) {
  ...
  try {
    ...
    return { counts, reasons: reasonCounts, gespeichert_at: saved.gespeichert_at || null };
  } catch (e) { /* Tag nicht lesbar — bei 0 anfangen */ }
  return { counts, reasons: reasonCounts, gespeichert_at: null };
}

export function saveDay(prefix, key, counts, reasonCounts) {
  try {
    // Der Zeitpunkt gehört dazu: nur mit ihm lässt sich später sagen, ob
    // dieser Stand älter ist als eine Korrektur auf dem Server (siehe
    // wasGiltJetzt).
    localStorage.setItem(prefix + key, JSON.stringify({
      counts, reasons: reasonCounts, gespeichert_at: new Date().toISOString(),
    }));
  } catch (e) {
...
export function alleGespeichertenTage(prefix, reasons) {
  const tage = [];
  let anzahl = 0;
  try { anzahl = localStorage.length; } catch (e) { return tage; }
  for (let i = 0; i < anzahl; i++) {
    // Jeder Eintrag für sich: ein einziger unlesbarer Schlüssel hat bisher
    // die ganze Schleife abgebrochen, und alles danach fehlte beim
    // Nachtragen — obwohl der Kommentar "überspringen" versprach.
    try {
      const fullKey = localStorage.key(i);
      if (!fullKey || !fullKey.startsWith(prefix + KEY_PREFIX)) continue;
      const tag = fullKey.slice((prefix + KEY_PREFIX).length);
      // Nur echte Tagesschlüssel. Daneben liegen weitere Einträge desselben
      // Tages (etwa der Anruf-Verlauf), die hier nichts zu suchen haben.
    ...
    } catch (e) { /* dieser Eintrag nicht lesbar — der nächste vielleicht schon */ }
  }
  return tage.sort((a, b) => a.tag.localeCompare(b.tag));
}

/**
 * Was gilt: der zusammengeführte Stand — oder der vom Server?
 *
 * Der Normalfall ist das Maximum je Zähler: zwei Geräte sollen sich nicht
 * gegenseitig löschen, und ein Zähler wächst ohnehin nur.
 *
 * Bei einer KORREKTUR ist das genau falsch. Wer eine zu hohe Zahl
 * heruntersetzt, bekam sie vom nächsten Gerät, auf dem noch der alte Stand
 * lag, sofort wieder hochgezogen — die Korrektur musste an jedem Gerät
 * wiederholt werden, und bis dahin sahen alle anderen den falschen Wert.
 *
 * Deshalb: Ist die Korrektur auf dem Server JÜNGER als der Stand dieses
 * Geräts, gilt der Server vollständig. Ohne Korrektur bleibt es beim
 * Maximum.
 *
 * @param {object} lokal  { counts, reasons, gespeichert_at }
 * @param {object} server { counts, reasons, korrigiert_at }
 */
export function wasGiltJetzt(lokal = {}, server = null) {
  if (!server) return { counts: lokal.counts || {}, reasons: lokal.reasons || {}, quelle: "lokal" };

  const korrigiert = server.korrigiert_at ? Date.parse(server.korrigiert_at) : null;
  const gespeichert = lokal.gespeichert_at ? Date.parse(lokal.gespeichert_at) : null;

  // Eine Korrektur, die neuer ist als alles, was dieses Gerät weiss —
  // einschliesslich des Falls, dass das Gerät gar keinen Zeitpunkt hat
  // (alter Eintrag von vor dieser Änderung).
  const korrekturGiltVor = korrigiert !== null && (gespeichert === null || korrigiert > gespeichert);
  if (korrekturGiltVor) {
    return { counts: server.counts || {}, reasons: server.reasons || {}, quelle: "server" };
  }

  return {
    counts: zaehlerZusammenfuehren(lokal.counts || {}, server.counts || {}),
    reasons: zaehlerZusammenfuehren(lokal.reasons || {}, server.reasons || {}),
    quelle: "zusammengefuehrt",
  };
}
