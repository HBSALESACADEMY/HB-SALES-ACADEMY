...
export function bestaetigungsText(lead, vertrieblerName = "", schritt = null) {
  const was = schritt?.vorStufe
    ? TERMIN_ARTEN.find((a) => a.key === schritt.vorStufe)?.label || schritt.label
    : artVon(lead).label;
  ...
  return `✅ ${was} mit ${lead?.name || "einem Kontakt"}${firma}${wer} bestätigt — findet am ${wann} statt.`;
}
