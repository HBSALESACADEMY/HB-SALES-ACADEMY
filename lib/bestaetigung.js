 ...
 *
 * Die Stufe kommt aus dem abgehakten SCHRITT, nicht aus dem Termin. Wer
 * die Closing-Bestätigung setzt, während der Termin noch auf der
 * Setting-Stufe steht, bekäme sonst "Setting Call bestätigt" gemeldet —
 * also das Gegenteil dessen, was er getan hat.
 */
export function bestaetigungsText(lead, vertrieblerName = "", schritt = null) {
  const wer = vertrieblerName ? ` von ${vertrieblerName}` : "";
  const firma = lead?.company ? ` (${lead.company})` : "";
  const kunde = `${lead?.name || "einem Kontakt"}${firma}${wer}`;

  // Eine Bestätigung schaut nach VORN — der Termin kommt noch, und wann er
  // ist, muss dabeistehen. Der Check-in schaut zurück: er ist erledigt,
  // und ein "findet statt am" wäre dort schlicht falsch.
  if (!schritt?.vorStufe) {
    return `✅ ${schritt?.label || "Schritt"}: ${kunde}.`;
  }

  const was = TERMIN_ARTEN.find((a) => a.key === schritt.vorStufe)?.label || schritt.label;
  const wann = lead?.appointment_at ? `${deutscheZeit(lead.appointment_at)} Uhr` : "einem offenen Zeitpunkt";
  return `✅ ${was} mit ${kunde} bestätigt — findet am ${wann} statt.`;
}

/**
 * Der Text zu einem erledigten Follow-up.
 *
 * In denselben Kanal wie die Terminbestätigungen: es ist dieselbe Frage —
 * hat sich jemand gekümmert. Ein eigener Kanal dafür hiesse, zwei
 * Gruppen im Blick behalten zu müssen, um eine Antwort zu bekommen.
 */
export function followUpErledigtText(nachfass, name = "") {
  const wer = name ? ` von ${name}` : "";
  const wann = nachfass?.faellig_am ? `, fällig war ${deutscheZeit(nachfass.faellig_am)} Uhr` : "";
  return `✅ Follow-up erledigt${wer}: ${nachfass?.titel || "ohne Titel"}${wann}.`;
}
