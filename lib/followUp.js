...
// Wie lange etwas liegen darf, bevor es auffällt.
//
// Eine lange Liste, in der alles gleich aussieht, ist keine Liste, sondern
// eine Wand. Drei Gruppen genügen: Was zwei Wochen liegt, ist ein anderer
// Fall als etwas von gestern — und wer das sieht, fängt oben an.
//
// Die Grenzen sind dieselben, mit denen die Zeile schon vorher eingefärbt
// wurde (14 und 7 Tage). Sie stehen jetzt hier, damit Farbe und Gruppe
// nicht auseinanderlaufen können.
export const DRINGLICHKEIT = [
  { key: "ueberfaellig", label: "Überfällig", abTagen: 14, ton: "coral", hinweis: "Liegt zwei Wochen oder länger" },
  { key: "faellig", label: "Wird alt", abTagen: 7, ton: "amber", hinweis: "Liegt über eine Woche" },
  { key: "frisch", label: "Frisch", abTagen: 0, ton: "muted", hinweis: "Aus den letzten sieben Tagen" },
];

/** In welche Dringlichkeitsgruppe ein Eintrag gehört. */
export function dringlichkeitVon(lead, jetzt = new Date()) {
  const tage = liegtSeit(lead, jetzt);
  if (tage === null) return DRINGLICHKEIT[DRINGLICHKEIT.length - 1];
  return DRINGLICHKEIT.find((d) => tage >= d.abTagen) || DRINGLICHKEIT[DRINGLICHKEIT.length - 1];
}

/**
 * Die Einträge nach Dringlichkeit gebündelt — dringendste Gruppe zuerst,
 * innerhalb jeder Gruppe das Älteste oben.
 *
 * Leere Gruppen fallen weg: Eine Überschrift ohne Inhalt kostet Platz und
 * sagt nichts.
 */
export function gruppiereNachDringlichkeit(leads = [], jetzt = new Date()) {
  return DRINGLICHKEIT
    .map((gruppe) => ({
      ...gruppe,
      leads: sortiereNachDringlichkeit(
        leads.filter((l) => dringlichkeitVon(l, jetzt).key === gruppe.key), jetzt,
      ),
    }))
 ...
 *
 * "aeltester" ist die Zahl, die weh tut — und genau deshalb steht sie da.
 */
export function uebersicht(leads = [], jetzt = new Date()) {
  const tage = leads.map((l) => liegtSeit(l, jetzt)).filter((t) => t !== null);
  const schwelle = DRINGLICHKEIT[0].abTagen;
  return {
    gesamt: leads.length,
    ueberfaellig: tage.filter((t) => t >= schwelle).length,
    aeltester: tage.length ? Math.max(...tage) : null,
    schwelle,
  };
}
