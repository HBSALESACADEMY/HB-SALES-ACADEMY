...
export async function erinnereAnNachfassTermine(admin, jetzt = new Date()) {
  // Bis zum Ende des heutigen Tages: die Erinnerung läuft morgens, und ein
  // Rückruf um 15 Uhr soll nicht erst am Folgetag gemeldet werden.
  ...
  const { data: faellig } = await admin.from("nachfass_termine")
    ...
    .lte("faellig_am", bis.toISOString())
    .limit(500);
