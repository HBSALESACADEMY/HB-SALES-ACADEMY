...
import { impulsFuer, impulsLage, impulsZeilen } from "./impulse.js";
...
export function lobZeilen({ heute, vorher, bestwerte = [], berichtTag, vergleichTag }) {
  ...
  lob.push("", zitatZeile(zitatFuer(zitatAnlass({ heute: h, vorher: v, bestwerte }), berichtTag)));
  return lob;
}

/**
 * Wozu das Zitat passen soll — das Wichtigste zuerst.
 *
 * Ein neuer Kunde schlägt alles andere: An dem Tag geht es um den
 * Abschluss, nicht darum, dass es auch drei Anwahlen mehr waren.
 */
export function zitatAnlass({ heute, vorher, bestwerte = [] }) {
  const h = heute || leereZahlen();
  const v = vorher || leereZahlen();
  if (h.kunden > 0) return "abschluss";
  if (bestwerte.length) return "spitze";
  if (h.followups > v.followups || h.mails > v.mails) return "nachfassen";
  if (KENNZAHLEN.some((k) => h[k.key] > v[k.key])) return "steigerung";
  return "dranbleiben";
}

/** Die ganze Nachricht. */
export function auswertungsText({ name = "", berichtTag, vergleichTag, heuteTag, heute, vorher, bestwerte = [] }) {
  const h = heute || leereZahlen();
  const v = vorher || leereZahlen();
  ...
  // Eine Zeile erscheint nur, wenn an einem der beiden Tage etwas darin
  // steht — und eine Gruppe nur mit mindestens einer Zeile. Wer keine Mails
  // schreibt, liest sonst jeden Morgen "Mails verschickt: 0 (Fr: 0)".
  let gruppe = null;
  KENNZAHLEN.forEach((k) => {
    if (!(h[k.key] > 0 || v[k.key] > 0)) return;
  ...
  // Zum Schluss der Impuls: Er gilt für den Tag, der jetzt anfängt, nicht
  // für den ausgewerteten. Am Montag steht also oben der Freitag und unten
  // der Montag.
  const fuerHeute = heuteTag || berichtTag;
  const lage = impulsLage({
    heute: h, vorher: v, bestwerte, heuteTag: fuerHeute,
    kennzahlen: KENNZAHLEN.map((k) => k.key),
  });
  zeilen.push("", ...impulsZeilen(impulsFuer(lage, fuerHeute)));
