...
export function rueckblickAnweisung({ name = "" } = {}) {
  return [
    ...
    '{"herausforderungen": ["…", "…"], "stimmung": "gut|gemischt|schwer", "vorhaben": "…", "zusammenfassung": "…", "uebung_gemacht": "ja|nein|unklar"}',
    ...
    "\"uebung_gemacht\": Wurde eine Übung besprochen und hat die Person sie umgesetzt? Nur \"ja\" oder \"nein\", wenn es klar dasteht, sonst \"unklar\".",
...
export function leseRueckblick(text) {
  ...
  // Nur ein klares Ja oder Nein zählt — "unklar" heisst: nicht wissen ist
  // ehrlicher als raten.
  const uebung = String(daten.uebung_gemacht || "").toLowerCase();
  const uebungGemacht = uebung === "ja" ? true : uebung === "nein" ? false : null;
  if (!herausforderungen.length && !stimmung && !vorhaben && !zusammenfassung) return null;
  return { herausforderungen, stimmung, vorhaben, zusammenfassung, uebungGemacht };
}
