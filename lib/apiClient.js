...
import { liesAntwort } from "./antwortLesen.js";
...
// Das Lesen der Antwort steht in lib/antwortLesen.js — ohne fetch und
// deshalb prüfbar. Hier bleibt nur der Weg nach draussen.
async function liesJson(res, pfad) {
  const text = await res.text().catch(() => "");
  const { daten, fehler, netz } = liesAntwort(text, res.status, pfad);
  if (fehler) {
    const e = new Error(fehler);
    // Damit die Anzeige unterscheiden kann: Ein Verbindungsabbruch ist
    // nichts, was der Betreiber beheben könnte.
    e.netz = !!netz;
    e.status = res.status;
    throw e;
  }
...
export async function apiPost(path, body) {
  ...
  const data = await liesJson(res, path);
  if (!res.ok) throw new Error(data.error || "Anfrage fehlgeschlagen.");
  return data;
}

// Ändern und Entfernen. Dieselbe Erneuerung der Anmeldung wie beim Senden —
// eine abgelaufene Sitzung soll nicht als "Anfrage fehlgeschlagen" enden.
