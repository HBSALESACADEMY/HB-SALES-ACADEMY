...
export async function sendeBegruessung(admin, userId, chatId) {
  ...
  // Vermerken, dass sie raus ist — sonst käme sie im Morgenlauf noch einmal
  // (lib/buddyErklaerungVersand.js).
  const { error } = await admin.from("telegram_verknuepfungen")
    .update({ erklaerung_am: new Date().toISOString() }).eq("user_id", userId);
  if (error) console.error("Erklärung nicht vermerkt:", error.message);
  return erste;
}
