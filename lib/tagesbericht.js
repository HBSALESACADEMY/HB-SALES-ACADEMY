...
export async function baueTagesbericht(admin) {
  // Gestern IN BERLIN. Vorher rechnete das mit der Uhr des Servers, und
  // die läuft in UTC: "gestern" begann um 01:00 oder 02:00 deutscher Zeit,
  // alles davor zählte zum Vortag, und die letzte Sekunde vor Mitternacht
  // fiel ganz heraus.
  const heuteTag = berlinHeute();
  const gesternTag = tagPlus(heuteTag, -1);
  const vonISO = tagesBeginnZeitpunkt(gesternTag);
  const bisISO = tagesBeginnZeitpunkt(heuteTag);
  ...
  const [{ data: orgs }, { data: profile }, { data: anrufe }, { data: termine }, { data: pruefungen }, { data: aufgaben }] =
    await Promise.all([
      ...
      admin.from("leads").select("created_by, created_at").is("geloescht_am", null).gte("created_at", vonISO).lt("created_at", bisISO),
      admin.from("exam_results").select("user_id, passed, created_at").gte("created_at", vonISO).lt("created_at", bisISO),
  ...
  const datum = new Date(`${gesternTag}T12:00:00Z`).toLocaleDateString("de-DE", { timeZone: "Europe/Berlin", weekday: "long", day: "2-digit", month: "long" });
  ...
  zeilen.push("");
  if (system.gesund) {
    zeilen.push("✅ System läuft.");
  } else {
    // Nur die Namen zu melden hiess: nachsehen gehen, um überhaupt zu wissen,
    // was kaputt ist. Folge und Behebung kommen deshalb mit in die Nachricht.
    zeilen.push("🔴 Störung erkannt:");
    system.pruefungen.filter((p) => !p.ok).forEach((p) => {
      zeilen.push(`• ${p.name}: ${p.hinweis}`);
      if (p.folge) zeilen.push(`  Folge: ${p.folge}`);
      if (p.loesung) zeilen.push(`  Behebung: ${p.loesung}`);
    });
  }
