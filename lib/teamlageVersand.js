...
import { tagPlus, berlinHeute } from "./woche.js";
import { arbeitstagDavor } from "./tagesauswertung.js";
...
export async function teamStand(admin, orgId, { jetzt = new Date() } = {}) {
  ...
  // Auch die Tage einzeln: Fragt die Leitung nach "heute" oder "gestern",
  // darf der Buddy nicht die Wochensumme nennen, weil er nichts anderes hat.
  const heuteTag = berlinHeute(jetzt);
  const letzterTag = arbeitstagDavor(heuteTag);
  ...
  if (!ids.length) return { woche, heuteTag, letzterTag, personen: [] };
  ...
  const [proTag, rueckblicke, schulungen, verknuepfungen, antworten, onboarding] = await Promise.all([
    zahlenFuerTage(admin, [...diese, ...vorher, heuteTag, letzterTag]),
  ...
  const personen = (mitglieder || []).map((m) => {
    ...
    return {
      ...
      heute: summiereTage(proTag, [heuteTag], m.id),
      letzterTag: summiereTage(proTag, [letzterTag], m.id),
  ...
  return { woche, heuteTag, letzterTag, personen };
}
