// Stimme und Sprechweise je Kundentyp (siehe pages/api/roleplay-voice.js).
//
// Eine einzige Stimme für alle klingt nach Ansage, nicht nach Kundschaft:
// der Choleriker muss anders klingen als der Unentschlossene, sonst nimmt
// man beiden die Rolle nicht ab. Die Namen sind die von Gemini
// bereitgestellten Stimmen; der Hinweis davor steuert die Sprechweise.
export const STIMMEN = {
  skeptiker:      { stimme: "Charon", art: "nüchtern, zurückhaltend und leicht misstrauisch, mit kurzen Pausen" },
  preisdruecker:  { stimme: "Puck",   art: "forsch, ungeduldig und geschäftsmässig, mit Betonung auf Zahlen" },
  techniker:      { stimme: "Orus",   art: "sachlich, präzise und ruhig, wie jemand, der genau zuhört" },
  unentschlossen: { stimme: "Aoede",  art: "zögerlich und nachdenklich, mit kleinen Denkpausen" },
  choleriker:     { stimme: "Fenrir", art: "gereizt, laut und ungeduldig, aber verständlich" },
  vielredner:     { stimme: "Leda",   art: "lebhaft, schnell und freundlich, wie jemand, der gern erzählt" },
};

export function stimmeFuer(personaId) {
  return STIMMEN[personaId] || { stimme: "Kore", art: "natürlich und freundlich" };
}
