...
export default function CallTracker() {
  ...
  // Der Tracker läuft als eigenständiges HTML im iframe, außerhalb von
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cached) {
        if (mounted) { setLogoUrl(cached.logo_url || null); setOrgName(cached.name || null); setLogoLoaded(true); }
        return;
      }
